...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lhs_cleartext: Vec<u64> = read_numbers_from_file(&Path::new(&("datasets/".to_owned() + &args[1] + "/lhs.txt")))?;
    let rhs_cleartext: Vec<u64> = read_numbers_from_file(&Path::new(&("datasets/".to_owned() + &args[1] + "/rhs.txt")))?;
